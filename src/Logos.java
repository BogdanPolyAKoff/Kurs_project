...
public class Logos implements Serializable {
    ...
    public void setValueFieldLeft(int value){                // устанавливает значение поля "Осталось до замены", меняет цвет: желтый - менее 30% от ресурса пробега, красный - менее 10%
        ...
        if (resource*1>value&&resource*0.4<=value){fieldLeft.setBackground(Color.green);}
        else if (resource*0.4>value&&resource*0.1<=value){fieldLeft.setBackground(Color.yellow);}
                ...
                try {
                    ...
                    for (Tabs tab:listTabs){   //
                        if ((publicPanel.getName()).equals(tab.getPublicPanel().getName())){
                            ...
                            for(Logos box:tab.getListBoxes()){          //реализует изменение последнего "осталось до замены" поля всех боксов
                ...
                try{
                    ...
                    for (Tabs tab:listTabs){
                        if ((publicPanel.getName()).equals(tab.getPublicPanel().getName())){
                            ...
                            for(Logos box:tab.getListBoxes()){                            //реализует изменение последнего поля всех боксов
                                int value = box.getValueFieldLastReplace()+box.getValueFieldResource()-dist;
                                box.setValueFieldLeft(value);
                            }
                            break;}
